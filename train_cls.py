...
def get_next_config_number(config_dir='configs/experiment/cls_experiment'):
    """다음 설정 파일 번호를 찾아 반환"""
    i = 1
    while True:
...
def main():
    parser = argparse.ArgumentParser(description='YOLO 분류 모델 학습')
    ...
    parser.add_argument(
        ...
        help='설정 파일 경로 (default: configs/default.yaml)'
    )
    ...
    if overrides:
        ...
        exp_dir = Path("configs") / "experiment" / "cls_experiment" / str(config_num)
        exp_dir.mkdir(parents=True, exist_ok=True)
        new_config_path = exp_dir / f"cls_experiment{config_num}.yaml"
        new_log_path = exp_dir / f"cls_experiment{config_num}.log"
